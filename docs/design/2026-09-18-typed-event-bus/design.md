...
**Model-slot typing.** The element/selection slot is fully recovered by canvas's existing `GroupSelection` (`canvas/src/types.ts:34`). For the _definition_ slot, **reuse canvas's existing `RegisteredElement`** (`{ id?; [key: string]: unknown }`) rather than minting a new shadow interface — core's `ModellingModelElement` / `DiagramElement` already structurally satisfy it (`DiagramElement extends RegisteredElement`, `canvas/src/types.ts:14`). Rich model precision on the _subscribe_ side (typed `.position`, `.waypoint`, etc.) is delivered by each callback's own parameter annotation (e.g. `drag.ts:159`, `SearchPanel.ts:75-84`), which eventemitter3's bivariant `on` listener checking already permits — **not** by the map. The map's honest guarantee is emit-site key + arity + primitive-payload checking; that is exactly the safety C12 targets (ROADMAP:480 — "the compiler, not review, catches a dispatcher wired to the wrong payload").

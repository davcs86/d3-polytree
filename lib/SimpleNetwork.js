...
var assign = require('lodash/object').assign,
  ...
  attachedDataTable = require('./utils/attachedDataTable');
...
SimpleNetwork.prototype.initSVG = function(){
  ...
  this.tooltip = d3js.tip()
    ...
    .offset([-10, 0])
    .html(function(d) {
      return isUndefined(d.label)?'':escapeHtml(d.label);
    });
...
SimpleNetwork.prototype.restart = function(recalculate){
  ...
  this.node.append('foreignObject')
    ...
    .each(function (d) {
      // Build the table as DOM nodes (textContent) instead of injecting an
      // HTML string, so attachedData values cannot execute in the host page.
      this.appendChild(attachedDataTable(d, that.options.tableHeaders));
    });

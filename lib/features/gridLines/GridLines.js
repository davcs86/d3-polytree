...
GridLines.prototype._init = function() {
  ...
  this._eventBus.on('canvas.zoomed', function(evt, deltaX, deltaY){
...
GridLines.prototype._draw = function(dX, dY) {
  ...
  _svg.selectAll('.hline') // draw horizontal lines
    ...
    .attr('y1', function (d) {
      return d * that._options.gridSize + ((translateY * -1.0)/scaleY) + (acumDeltaY % that._options.gridSize)/scaleY;
    })
    .attr('y2', function (d) {
      return d * that._options.gridSize + ((translateY * -1.0)/scaleY) + (acumDeltaY % that._options.gridSize)/scaleY;
    })
  ...
  _svg.selectAll('.vline')  // draw vertical lines
    ...
    .attr('x1', function (d) {
      ...
      return d * that._options.gridSize + ((translateX * -1.0)/scaleX) + (acumDeltaX % that._options.gridSize)/scaleX;
    })
    .attr('x2', function (d) {
      return d * that._options.gridSize + ((translateX * -1.0)/scaleX) + (acumDeltaX % that._options.gridSize)/scaleX;
    })

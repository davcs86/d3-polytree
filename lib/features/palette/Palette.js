...
Palette.prototype._createDelegates = function(){
  ...
  // prevent drag propagation
  domDelegate.bind(this._canvas.getContainer(), '.pfdjs-entry', 'dragstart', function(event) {
    var rects = event.srcElement.getBoundingClientRect();
    event.lastPosition = lastPosition = {x: rects.left, y: rects.top };
    event.delta = delta = {x: 0, y: 0};

    that.trigger('dragstart', event);
  });

  domDelegate.bind(this._canvas.getContainer(), '.pfdjs-entry', 'dragend', function(event) {
  ...
  domDelegate.bind(this._canvas.getContainer(), '.pfdjs-entry', 'drag', function(event) {
    var rects = {left: event.pageX, top: event.pageY};
    event.delta = {x: rects.left - lastPosition.x, y: rects.top - lastPosition.y};
    event.lastPosition = lastPosition = {x: rects.left, y: rects.top };

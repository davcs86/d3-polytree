...
Viewer.prototype._init = function (graph) {
  ...
  that.get('eventBus').emit('diagram.ready');
};
...
Viewer.prototype._createGraph = function (options) {
  ...
  var graph = this.graph = new Graph(options);
  this._init(graph);
  return graph;
};
...
Viewer.prototype.exportDiagram = function (options) {
  ...
  try {
    ...
    this.moddle.toXML(definitions, options, function (err, xmlString) {
      if (err) {
        deferResponse.reject(err);
        return;
      }
      deferResponse.resolve(xmlString);
    });
  } catch (e) {
    deferResponse.reject(e);
  }
...
Viewer.prototype.exportSVG = function () {
  var deferResponse = Deferred();
  ...
  try {
    ...
    var svgString =
      ...
      '<!-- created with d3-polytree / https://github.com/davcs86/d3-polytree -->\n' +
      this.get('canvas').getSVGStr();

    deferResponse.resolve(svgString);

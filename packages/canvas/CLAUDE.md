...
3. **Touch only what the task requires; keep diffs surgical.** _(enforced by `CANVAS-01` `false`-sentinel + `CANVAS-02` layer split; `PLAT-04`)_
4. **Define success up front, then loop until verified.** _(enforced by `CANVAS-03` SSR id contract; `PLAT-06` determinism)_

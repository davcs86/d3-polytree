...
- **`ElementBuilder.create` silently no-ops when `prefix === ''`** (a dedicated test locks it in), but the _why_ is unexplained — see findings open question before relying on it as an intentional "skip" signal. Evidence: `packages/canvas/src/ElementBuilder.ts#create`.

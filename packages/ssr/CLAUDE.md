...
3. **Touch only what the task requires; keep diffs surgical.** _(enforced by `SSR-02` — add-only jsdom install, restored in `finally`; `SSR-03` idGenerator token seam)_
4. **Define success up front, then loop until verified.** _(enforced by `SSR-01` pre-claim-before-boot; `PLAT-06` determinism)_

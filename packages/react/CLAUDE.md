...
3. **Touch only what the task requires; keep diffs surgical.** *(enforced by `REACT-01` — stable scalar snapshot; `REACT-02` — latest-props-via-ref, effect deps `[]`)*
4. **Define success up front, then loop until verified.** *(enforced by `REACT-N03` — no UMD pass by design)*

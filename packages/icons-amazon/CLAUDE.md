...
3. **Touch only what the task requires; keep diffs surgical.** _(enforced by `ICONS-01` — filename IS the node type; `PLAT-04` — regenerate, never hand-edit `icons.generated.ts`)_
4. **Define success up front, then loop until verified.** _(enforced by `ICONS-02` — pack spreads `{...createIcons(), ...pack}`, composed after core)_

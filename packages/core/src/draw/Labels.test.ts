...
describe('Labels', () => {
  ...
  it('renders a text label at its position', () => {
    ...
    const labels = new Labels([def], s.canvas, s.bus, s.elementBuilder, s.elementRegistry, s.drawingRegistry);
  ...
  it('updates the text on update', () => {
    const s = makeServices();
    const def = makeDef('l1', { position: { x: 0, y: 0 }, text: 'a', fontSize: 10 }) as unknown as LabelDefinition;
    const labels = new Labels([def], s.canvas, s.bus, s.elementBuilder, s.elementRegistry, s.drawingRegistry);
    def.text = 'b';

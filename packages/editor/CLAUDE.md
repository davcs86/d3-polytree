...
1. **Don't assume — ask, and surface tradeoffs.** _(enforced by `EDITOR-01` — `element.updateProperties` is the one editor-owned command; `PLAT-03`)_
2. **Write the minimum that solves the stated problem.**
3. **Touch only what the task requires; keep diffs surgical.** _(enforced by `EDITOR-02` deepGet/deepSet undo capture + `EDITOR-03` module tier; `PLAT-04`)_
4. **Define success up front, then loop until verified.** _(enforced by `PLAT-01` notifications-last; `PLAT-06` determinism)_

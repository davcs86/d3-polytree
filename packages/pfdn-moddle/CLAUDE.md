...
3. **Touch only what the task requires; keep diffs surgical.** _(enforced by `MODDLE-02` IDREFs + `MODDLE-03` `xsi:type` on complex props; `PLAT-04`)_
4. **Define success up front, then loop until verified.** _(enforced by `MODDLE-04` — the byte-identical `toXML` round-trip oracle)_

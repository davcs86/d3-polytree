...
3. **Touch only what the task requires; keep diffs surgical.** _(enforced by `LAYOUT-02` — keep geometry abstract until the final projection)_
4. **Define success up front, then loop until verified.** _(enforced by `LAYOUT-01` deterministic tie-breaks; `PLAT-06`)_

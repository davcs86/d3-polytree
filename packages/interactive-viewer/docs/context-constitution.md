...
- **Search fell back to element id + refreshes on `node.updated`/`link.updated`** because a node's label text is set *after* `node.created`, so newly added nodes never appeared. Don't re-narrow search to caption-only. Scar PR #70 (`8394296`).
